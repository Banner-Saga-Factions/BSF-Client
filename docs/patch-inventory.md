...
| `src/engine/mod/ModBattleControl.as` | **New file.** Five bridge commands: `battle_state` reads the live battle (units, sides, tiles, current-versus-base stats), `battle_deploy_ready` gets past the deploy screen (which an offline battle never leaves on its own), `battle_end_turn` ends the turn in progress, `battle_move` walks the active unit to a named tile, and `battle_attack` has it swing at another unit. Registered from `GameFsm`. Reaches the battle through one untyped hop so an engine-layer class need not depend on game-layer `SceneState`. Each of the four that change something makes the same calls the mouse would, so in an **offline** battle none of them reaches the server — used on a local turn in an online battle they send exactly what the mouse would. | Lets a test play and check a whole battle without synthesising mouse clicks — see [`driving-the-client.md`](./driving-the-client.md) → "Two channels". |
