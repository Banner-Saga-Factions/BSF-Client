...
- **The long-poll always hits `services/game/{sessionKey}`** (`TxnGet.as:13`). `HttpCommunicator.DEFAULT_POLL_TIME = 3000` (line 18) is a **gap between polls, not a request timeout** — it becomes `HttpAction.send`'s pre-send delay, so the client waits 3 s and then polls (1 s during an **online** battle; an offline practice battle keeps the 3 s gap). `bsf-server` holds each poll up to **5 s**. See [`wire-protocol.md`](./wire-protocol.md) → "Long-poll mechanics".
