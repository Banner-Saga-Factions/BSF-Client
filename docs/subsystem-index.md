...
| Class                                                                                                                      | Role                                                                                                                                            | Decompile path                                        | 2013 path                                                            |
| -------------------------------------------------------------------------------------------------------------------------- | ----------------------------------------------------------------------------------------------------------------------------------------------- | ----------------------------------------------------- | -------------------------------------------------------------------- |
| `BattleFsm` †                                                                                                              | Battle finite-state machine. Tightens HTTP poll to 1000 ms via `setPollTimeRequirement(this, 1000)` on `startFsm` (line 374).                   | `engine/battle/fsm/BattleFsm.as`                      | (stale — use decompile)                                              |
...
| `NullSteamworks`          | Complete no-op `ISteamworks` implementation. **Crossplay subclasses this** as `DiscordSteamworks` (see `bsf-client/src/`). | `engine/steamworks/NullSteamworks.as`          | (not in 2013) |
...
| `states/PreAuthState` ★ | Login state. Calls Steam to get auth ticket; if `overrideSteamId` is set, supplies `"override-authticket"`. **The crossplay patch lives here** (replace the bypass string with a real Discord OAuth token). The only file currently in `bsf-client/src/`. |
| `states/` (other)       | `OfflineState`, `OnlineState`, `LoginState`, etc. — the GameFsm states.                                                                                                                                                                                   |
...
| Subpackage             | Role                                                                                                                                                                                                                                                                                                |
| ---------------------- | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `tbs/srv/battle/data/` | Battle message DTOs (`BattleReadyData`, `BattleDeployData`, `BattleSyncData`, `BattleMoveData`, `BattleActionData`, `BattleKilledData`, `BattleFinishedData`). Authoritative when a Fiddler capture or [`bsf-server/docs/dataStructures.md`](../../bsf-server/docs/dataStructures.md) is ambiguous. |
